...
export const AdminSidebar: React.FC<AdminSidebarProps> = ({ currentView, setCurrentView, onClose, onCollapse, onMobileClose }) => {
    ...
    const buttonClass = (view: AdminView) => `
        ...
        ${currentView === view 
            ? `bg-gradient-to-r from-fuchsia-600/20 to-purple-600/10 text-fuchsia-300 ${language === 'ar' ? 'border-r-4' : 'border-l-4'} border-fuchsia-500 shadow-lg shadow-fuchsia-900/20` 
    ...
    return (
        <div className={`w-72 bg-[#1e0b2b] flex-shrink-0 flex flex-col h-full ${language === 'ar' ? 'border-l' : 'border-r'} border-fuchsia-900/30 shadow-2xl`}>
            {/* Header */}
            <header className="relative p-6 border-b border-fuchsia-500/20 flex flex-col items-center justify-center gap-y-2 bg-gradient-to-b from-fuchsia-900/20 to-transparent">
                <div className="w-12 h-12 rounded-full bg-gradient-to-tr from-fuchsia-600 to-purple-600 flex items-center justify-center shadow-lg shadow-fuchsia-500/30">
                    <ShieldCheckIcon className="w-6 h-6 text-white"/>
                </div>
                <h1 className="text-lg font-black text-transparent bg-clip-text bg-gradient-to-r from-white to-fuchsia-200 tracking-wider mt-2">{t('adminHeader.title')}</h1>
            ...
            <nav className="flex-grow p-4 space-y-2 overflow-y-auto custom-scrollbar">
                {menuItems.map(({ view, icon: Icon, labelKey }) => (
                    <button key={view} onClick={() => setCurrentView(view)} className={buttonClass(view)}>
                        <Icon className={`w-5 h-5 flex-shrink-0 transition-transform duration-300 group-hover:scale-110 ${currentView === view ? 'text-fuchsia-400' : 'text-slate-500 group-hover:text-white'}`} />
            ...
            {/* Footer Actions */}
             <footer className="hidden md:flex p-4 border-t border-fuchsia-500/20 mt-auto items-center justify-center gap-4 bg-black/20">
