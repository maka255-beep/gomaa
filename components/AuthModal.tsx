...
const AuthModal: React.FC<AuthModalProps> = ({ isOpen, onClose, onSuccess, isClosable = true, onBack, showRegisterView = true, loginOnlyTitle, loginButtonText = 'دخول', initialView = 'login' }) => {
  ...
  const handleRegister = (e: React.FormEvent) => {
    ...
    if (isGulfOrArab && phone.length < 8) {
      setError('للدول الخليجية والعربية، يجب ألا يقل رقم الهاتف عن 8 أرقام. مثل: 5XXXXXXX');
      return;
    }
  ...
  const handleLogin = (e: React.FormEvent) => {
    ...
    if (result.user) {
      ...
      if (result.error === 'concurrent_session') {
        ...
        setError('البريد الإلكتروني أو رقم الهاتف غير صحيح. يرجى المحاولة مرة أخرى أو إنشاء حساب جديد.');
      }
    }
  };
