...
const LiveStreamCard: React.FC<LiveStreamCardProps> = ({ workshopId, workshopTitle, zoomLink, user, onLoginRequest, onZoomRedirect }) => {
    ...
    // FIX: Check if the user is subscribed (excluding donations)
    ...
    return (
        <div className="group relative max-w-lg mx-auto my-6 sm:my-8">
            {/* Outer animated glow - Royal Fuchsia & Gold mix */}
            <div className="absolute -inset-0.5 bg-gradient-to-r from-fuchsia-600 via-purple-500 to-indigo-600 rounded-2xl blur opacity-30 group-hover:opacity-60 transition duration-500 animate-pulse"></div>
            
            {/* Main card - Dark Royal Gradient Background */}
            <div className="relative bg-gradient-to-br from-[#2e0235] via-[#4c1d95] to-[#701a75] rounded-2xl shadow-2xl shadow-black/50 p-5 sm:p-6 text-center flex flex-col items-center transform group-hover:-translate-y-1 transition-transform duration-300 border border-fuchsia-500/30">

                {/* Icon section with pulsing effect */}
                <div className="relative mb-4">
                    <span className="absolute -inset-3 animate-ping rounded-full bg-fuchsia-500 opacity-10"></span>
                    <div className="relative bg-white/10 p-4 rounded-full border border-fuchsia-400/30 shadow-[0_0_15px_rgba(217,70,239,0.3)] backdrop-blur-md">
                        <VideoIcon className="w-8 h-8 sm:w-10 sm:h-10 text-fuchsia-200 drop-shadow-md"/>
                    </div>
                </div>
                ...
                {/* Action button - Updated to Royal Mauve Theme */}
                <button
                    onClick={handleLinkClick}
                    className="inline-flex items-center justify-center gap-x-2 bg-gradient-to-r from-fuchsia-600 to-purple-600 hover:from-fuchsia-500 hover:to-purple-500 text-white font-bold py-3 px-8 rounded-xl transition-all duration-300 transform hover:scale-105 shadow-lg shadow-fuchsia-500/30 hover:shadow-fuchsia-500/50 text-sm sm:text-base border border-fuchsia-500/20"
                >
                ...
                {/* Helper text */}
                {!user && <p className="text-fuchsia-200/60 text-center mt-4 text-[10px] sm:text-xs font-medium">يجب تسجيل الدخول أولاً للتحقق من اشتراكك.</p>}
                {user && !isSubscribed && (
